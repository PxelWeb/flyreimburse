...
class NomadApplicationDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action',function($query){
                $viewBtn ="<a href=".route('admin.nomad-application-details',$query->id)." class='btn btn-primary mr-2'><i class='fas fa-eye'></i></i></i></a>";
                return $viewBtn;
    ...
    {
        $query =  $model->where('user_id',Auth()->user()->id)->newQuery();
        ...
        if($this->request()->has('start_date') && $this->request()->has('end_date')){
            ...
            if($startDate && $endDate){
                //return all application who are between these 2 date startDate and endDate;
    ...
    {
        return $this->builder()
                    ->setTableId('nomad-application-table')
    ...
    {
        return 'AdminApplication_' . date('YmdHis');
    }
}
