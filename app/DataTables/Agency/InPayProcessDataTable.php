...
class InPayProcessDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action',function($query){
                $viewBtn ="<a href=".route('agency.application-details',$query->id)."  class='btn btn-primary mr-2'><i class='fas fa-eye'></i></i></i></a>";
                return $viewBtn;
    ...
    {
        $query =  $model->where('status','in_pay_process')
        ->where('user_id',Auth::user()->id)->newQuery();
    ...
    {
        return 'InPayProcess_' . date('YmdHis');
    }
}
